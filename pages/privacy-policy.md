# Privacybeleid — Garden by Jess

**Ingangsdatum:** 3 september 2026  
**Laatst bijgewerkt:** 3 september 2026  
**Verwerkingsverantwoordelijke:** Garden by Jess B.V., Plantsoenlaan 14, 1012 JS Amsterdam, Nederland  
**Contact:** `privacy@gardenbyjess.store` | `care@gardenbyjess.store`

---

## 1. Inleiding & Onze Privacybelofte
Bij **Garden by Jess** behandelen wij uw persoonsgegevens met hetzelfde respect, dezelfde zuiverheid en zorg als onze ambachtelijk geteeld kruidenoogsten. Dit privacybeleid legt op heldere en transparante wijze uit hoe wij uw persoonsgegevens verzamelen, verwerken en beveiligen wanneer u `gardenbyjess.store` bezoekt, een bestelling plaatst in ons atelier of contact opneemt met onze herboristen, in overeenstemming met de Algemene Verordening Gegevensbescherming (AVG / GDPR) en de Nederlandse Uitvoeringswet AVG.

---

## 2. Welke Gegevens Wij Verzamelen
Wij verzamelen uitsluitend de strikt noodzakelijke gegevens om uw bestellingen accuraat uit te voeren en u van deskundig advies te voorzien:
- **Bestel- en Transactiegegevens:** Volledige naam, afleveradres, factuuradres, e-mailadres, telefoonnummer en betalingsreferentie. *Let op: creditcard- en bankgegevens worden direct en versleuteld verwerkt door gecertificeerde PCI-DSS level-1 betaalproviders (zoals iDEAL via Mollie/Shopify Payments, Bancontact en Klarna). Garden by Jess heeft nimmer toegang tot uw volledige kaartnummers.*
- **Correspondentie & Advies:** Vragen over theezetmethoden, kruideninteracties en berichten verzonden aan `care@gardenbyjess.store`.
- **Technische & Statistische Gegevens:** IP-adres, browsertype, apparaatgegevens en bezochte pagina's (uitsluitend verwerkt via privacyvriendelijke en geanonimiseerde Google Analytics 4 metingen).

---

## 3. Grondslagen van Verwerking
Wij verwerken uw persoonsgegevens uitsluitend op basis van de wettelijke AVG-grondslagen (Artikel 6 AVG):
1. **Uitvoering van de Overeenkomst:** Het samenstellen, verpakken en laten bezorgen van uw kruidenthee via PostNL of DHL.
2. **Wettelijke Verplichting:** Het bewaren van fiscale facturen en aankoopbewijzen voor de wettelijke fiscale bewaartermijn van de Belastingdienst (7 jaar).
3. **Gerechtvaardigd Belang:** Het optimaliseren van onze recepturen, sitebeveiliging en fraudepreventie.
4. **Toestemming:** Voor de toezending van onze nieuwsbrief *De Tuinkroniek* of gepersonaliseerd zetadvies. U kunt uw toestemming op ieder gewenst moment met één klik intrekken via de uitschrijflink onderaan elke e-mail.

---

## 4. Delen met Derden (Verwerkers)
Wij delen uw gegevens nooit met derden voor commerciële of advertentiedoeleinden. Wij werken uitsluitend met betrouwbare partners:
- **Webwinkelplatform:** Shopify Inc. (veilige hosting en transactieverwerking).
- **Vervoersdiensten:** PostNL en DHL (voor de fysieke bezorging en track & trace).
- **Betaaldiensten:** Mollie / Stripe / Klarna (beveiligde transactie-afhandeling).

---

## 5. Uw Rechten Onder de AVG
Als betrokkene beschikt u over uitgebreide wettelijke privacyrechten:
- **Recht op Inzage:** U kunt opvragen welke gegevens wij van u bewaren.
- **Recht op Rectificatie:** U kunt onjuiste gegevens kosteloos laten corrigeren.
- **Recht op Vergetelheid (Verwijdering):** U kunt verzoeken om verwijdering van uw gegevens (voor zover niet strijdig met wettelijke fiscale bewaarplichten).
- **Recht op Beperking van de Verwerking:** U kunt de verwerking tijdelijk laten pauzeren.
- **Recht op Dataportabiliteit:** U kunt uw gegevens in een gestructureerd digitaal formaat opvragen.

Voor het uitoefenen van deze rechten kunt u een e-mail sturen naar `privacy@gardenbyjess.store`. Wij behandelen en bevestigen uw verzoek **binnen 24 uur**.

---

## 6. Beveiliging van Gegevens
Onze website maakt gebruik van end-to-end TLS 1.3 encryptie, strikte toegangscontrole en gehashte wachtwoorden. Uw gegevens worden opgeslagen op zwaarbeveiligde Europese servers.
